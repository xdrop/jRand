...
public class CardTypeGenerator extends Generator<CardType> {
    ...
    public final CardTypeGenerator fork() {
        return new CardTypeGenerator(
                new java.util.HashMap<>(all),
                allCol,
                new java.util.HashMap<>(defaultFormats),
                new java.util.ArrayList<>(customPool));
    }
    
    @Generated("me.xdrop.jrand.annotation.processing.ForkClassGenerator")
    private CardTypeGenerator(Map<String, CardType> all, CardType[] allCol,
            Map<Integer, String> defaultFormats, List<CardType> customPool) {
        this.all = all;
        this.allCol = allCol;
